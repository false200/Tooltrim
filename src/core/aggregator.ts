...
const PROXY_INFO = { name: "mcp-diet", version: "0.1.0" };
...
export class Aggregator {
  ...
  createServer(): Server {
    const server = new Server(PROXY_INFO, {
      ...
      instructions:
        "You're talking to mcp-diet, a proxy that aggregates multiple MCP servers into one. " +
  ...
  private wireHandlers(server: Server): void {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
      ...
      if (!route) {
        throw new Error(`tool "${name}" not found in mcp-diet routing table`);
      }
      if (!this.deps.filter.isAllowed(name, "tool")) {
        throw new Error(`tool "${name}" is denied by mcp-diet policy`);
      }
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
      ...
      if (!upstreamId) {
        throw new Error(`resource "${uri}" not found in mcp-diet routing table`);
      }
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (req) => {
      ...
      if (!route) {
        throw new Error(`prompt "${name}" not found in mcp-diet routing table`);
      }
      const conn = this.deps.upstream.connections.get(route.upstreamId);
